...
using Application.Features.Employees.Commands.TerminateOfEmployee;
...
public class EmployeeController : BaseController
{
    ...
    [HttpPost]
    public async Task<IActionResult> TerminateOfEmployee(TerminateOfEmployeeCommand terminateOfEmployeeCommand)
    {
        var result = await Mediator!.Send(terminateOfEmployeeCommand);
        var employeeList = await Mediator.Send(new GetListEmployeeQuery() {PageRequest = new PageRequest(){Page = 0,PageSize = 5}});
        return View("Index",employeeList);
    }
    
    [HttpGet]
    public async Task<IActionResult> TerminationOfEmployee(int id)
    {
        var result = await Mediator!.Send(new GetByIdEmployeeQuery(id));
        return View("TerminateOfEmployee",result);
    }
